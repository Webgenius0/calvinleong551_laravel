...
class SkinToneService
{
    ...
    public function analyzeSkinTone(string $bridePath, string $groomPath, string $season): array
    {
        try {
            ...
            // Step 4: Compile all responses
            $allResponse = $this->generateAllResponse($season, $brideData, $groomData, $bridePath, $groomPath);
    ...
    private function generateSeasonTheme(string $season, array $brideData, array $groomData, string $bridePath, string $groomPath): array
    {
        ...
        Log::info("Generating season palette with prompt: {$palettePrompt}");
    ...
    // default color plate
    /**
     * Generate 4 color palettes as objects with titles and descriptions (no images).
     */
    /**
     * Generate 4 color palettes as objects with titles, descriptions and 6 images each.
     */
    private function generateAllResponse(string $season, array $brideData, array $groomData, string $bridePath, string $groomPath): array
    {
        $palettes = [];
        
        for ($i = 1; $i <= 4; $i++) {
            // Generate title and description
            $palettePrompt = "Generate a beautiful title and description for wedding color palette #{$i} for {$season} season, based on neutral tones and white colors. Title: 5 words max. Description: 2 sentences. Respond ONLY in JSON: {\"title\": \"Elegant Summer Glow\", \"description\": \"This palette...\"}";
            $paletteInfo = json_decode($this->callSuggestionModel($palettePrompt) ?? '{}', true) ?: [
            ...
            // Generate 4 colors
            $colorPrompt = "Suggest 4 hex colors for wedding palette #{$i} in {$season} theme, harmonious with white and neutral tones. Respond ONLY with JSON array: [\"#ff6b6b\", \"#4ecdc4\", \"#45b7d1\", \"#f9ca24\"]";
            $colors = json_decode($this->callSuggestionModel($colorPrompt) ?? '[]', true) ?: $this->getSeasonFallbackColors($season);

            // Generate 6 different images for this palette
            $images = $this->generatePaletteImages($season, $colors, $bridePath, $groomPath, $i);
     ...
     */
    private function generatePaletteImages(string $season, array $colors, string $bridePath, string $groomPath, int $paletteIndex): array
    {
        $images = [];
        
        // 1. Groom and bride wedding image
        $images[] = $this->generateCoupleWeddingImage($season, $colors, $bridePath, $groomPath, $paletteIndex);
        
        // 2. Groom individual season-based wedding image
        $images[] = $this->generateGroomIndividualImage($season, $colors, $groomPath, $paletteIndex);
        
        // 3. Groom with friends (4 persons)
        $images[] = $this->generateGroomFriendsImage($season, $colors, $groomPath, $paletteIndex);
        
        // 4. Bride with friends (4 persons)
        $images[] = $this->generateBrideFriendsImage($season, $colors, $bridePath, $paletteIndex);
        
        // 5. Couple praying image with color code
        $images[] = $this->generateCouplePrayingImage($season, $colors, $bridePath, $groomPath, $paletteIndex);
        
        // 6. Bride, groom and friends (6 persons)
        $images[] = $this->generateGroupFriendsImage($season, $colors, $bridePath, $groomPath, $paletteIndex);
     ...
     * 1. Groom and bride wedding image
     */
    private function generateCoupleWeddingImage(string $season, array $colors, string $bridePath, string $groomPath, int $paletteIndex): array
    {
        ...
        $imagePath = $this->saveImageToPublic($imageData, "couple_wedding_{$season}_{$paletteIndex}");
     ...
     * 2. Groom individual season-based wedding image
     */
    private function generateGroomIndividualImage(string $season, array $colors, string $groomPath, int $paletteIndex): array
    {
        ...
        $imagePath = $this->saveImageToPublic($imageData, "groom_individual_{$season}_{$paletteIndex}");
     ...
     * 3. Groom with friends (4 persons)
     */
    private function generateGroomFriendsImage(string $season, array $colors, string $groomPath, int $paletteIndex): array
    {
        ...
        $imageData = $this->generateImageWithFace($prompt, $groomPath);
        $imagePath = $this->saveImageToPublic($imageData, "groom_friends_{$season}_{$paletteIndex}");
     ...
     * 4. Bride with friends (4 persons)
     */
    private function generateBrideFriendsImage(string $season, array $colors, string $bridePath, int $paletteIndex): array
    {
        ...
        $imagePath = $this->saveImageToPublic($imageData, "bride_friends_{$season}_{$paletteIndex}");
     ...
     * 5. Couple praying image with color code
     */
    private function generateCouplePrayingImage(string $season, array $colors, string $bridePath, string $groomPath, int $paletteIndex): array
    {
        ...
        $imageData = $this->generateImageWithFaces($prompt, $bridePath, $groomPath);
        $imagePath = $this->saveImageToPublic($imageData, "couple_praying_{$season}_{$paletteIndex}");
     ...
     * 6. Bride, groom and friends (6 persons)
     */
    private function generateGroupFriendsImage(string $season, array $colors, string $bridePath, string $groomPath, int $paletteIndex): array
    {
        ...
        $imageData = $this->generateImageWithFaces($prompt, $bridePath, $groomPath);
        $imagePath = $this->saveImageToPublic($imageData, "group_friends_{$season}_{$paletteIndex}");
    ...
    // ... Keep all your existing methods (analyzeSingleImage, callSuggestionModel, getSeasonFallbackColors, etc.)
    ...
    private function getFallbackResponse(string $season): array
    {
        return [
            'bride' => ['skin_tone' => 'neutral', 'color_code' => ['#ffffff', '#f0f0f0', '#e0e0e0', '#d0d0d0'], 'matching_colors' => []],
            'groom' => ['skin_tone' => 'neutral', 'color_code' => ['#ffffff', '#f0f0f0', '#e0e0e0', '#d0d0d0'], 'matching_colors' => []],
            'season' => ['name' => ucfirst($season), 'palette' => ['#FFD700', '#87CEEB', '#98FB98', '#FFB6C1'], 'description' => ''],
            'all_responses' => []
        ];
    }
}
